...
function Detail() {
  ...
  const getPokemon = async () => {
    await api.get(`${API_URL}${env.POKEMON_URL}/${id}`).then((response) => {
  ...
  const getTypes = async (typ) => {
    let types = [];
    typ.forEach((t) => {
      api.get(t.type.url).then(async (response) => {
  ...
  const getSpecies = async (spe) => {
    await api
      .get(`${API_URL}${env.POKEMON_URL}-species/${spe}`)
      .then((response) => {
  ...
  const getHabitat = async (hab) => {
    await api.get(hab).then((response) => {
  ...
  const getDescriptions = async (des) => {
    ...
    if (texto === "" && des.length > 0) texto = des[0].flavor_text;
  ...
  const getAbilities = async (abil) => {
    let abilities = [];
    abil.forEach((a) => {
      api.get(a.ability.url).then(async (response) => {
  ...
  const getStatistics = async (sta) => {
    let statistics = [];
    sta.forEach((a) => {
      api.get(a.stat.url).then(async (response) => {
        statistics.push({
          name: response.data.names[5].name,
          base: a.base_stat,
        });
      });
    });
  ...
  const getEvolutions = async (evo) => {
    await api.get(evo).then(async (response) => {
              ...
              <Row className={cardClass}>
                <Col md="6">
                  <CardImg
                    src={imagen}
                    alt={pokemon.name}
                    className="w-100 img-fluid animate__animated animate__bounceInRight card-hover"
                  />
                </Col>
                <Col md="6">
                ...
                <Row>
                  {listEvolutions.map((evo, index) => (
                    <Col sd="3" key={index}>
                      <PokeCard key={index} pokemon={evo} sm="0" md="12" lg="0" />
                    </Col>
                  ))}
                </Row>
              </Row>
